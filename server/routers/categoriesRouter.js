...
router.post("/", async (req, res) => {
    try {
        ...
        const newCategory = await pool.query("INSERT INTO categories (category_name) VALUES ($1) RETURNING *", [category_name]);
...
router.put("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { category_name } = req.body;
        const updateCategory = await pool.query("UPDATE categories SET category_name = $1 WHERE id = $2", [category_name, id])
...
router.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const deleteCategory = await pool.query("DELETE FROM categories WHERE id = $1", [id]);
