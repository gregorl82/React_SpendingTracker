...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const merchant = await pool.query("SELECT * FROM merchants WHERE id = $1", [
      id,
    ]);
...
router.post("/", async (req, res) => {
  try {
    ...
    const newMerchant = await pool.query(
      "INSERT INTO merchants (merchant_name) VALUES ($1) RETURNING *",
      [merchant_name]
    );
...
router.put("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { merchant_name } = req.body;
    const updateMerchant = await pool.query(
      "UPDATE merchants SET merchant_name = $1 WHERE id = $2",
      [merchant_name, id]
    );
...
router.delete("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deleteMerchant = await pool.query(
